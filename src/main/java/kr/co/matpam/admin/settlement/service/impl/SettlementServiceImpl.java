...
public class SettlementServiceImpl extends EgovAbstractServiceImpl implements SettlementService {
    ...
    @Override
    @Transactional
    public void executeDailySettlement(String settleDate) throws Exception {
        // 1. 해당 일자의 기존 정산 데이터 삭제 (멱등성 보장)
        settlementDAO.deleteSettlementByDate(settleDate);

        // 2. 주문 정보 기반 정산 집계 수행 (MyBatis aggregation query)
        List<SettlementVO> settlementDataList = settlementDAO.aggregateSettlementData(settleDate);
