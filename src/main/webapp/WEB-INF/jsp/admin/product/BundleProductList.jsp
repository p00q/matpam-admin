...
<div class="container-fluid p-4">
    ...
    <div class="card mb-4 border-0 shadow-sm">
        <div class="card-body p-0">
            <form id="searchForm" method="get" action="<c:url value='/admin/product/bundleProductList.do'/>">
                ...
                <table class="table table-bordered mb-0 align-middle search-filter-table">
                    ...
                    <tbody>
                            ...
                            <td>
                                <select name="saleType" class="form-select form-select-sm" style="max-width: 200px;">
                                    ...
                                    <c:forEach var="code" items="${saleTypes}">
                                        <option value="${code.detailCode}" ${param.saleType eq code.detailCode ? 'selected' : ''}>${code.detailCodeName}</option>
                                    </c:forEach>
                                </select>
                            </td>
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="storageType" class="form-select form-select-sm" style="max-width: 200px;">
                                    <option value="">전체</option>
                                    <c:forEach var="code" items="${storageTypes}">
                                        <option value="${code.detailCode}" ${param.storageType eq code.detailCode ? 'selected' : ''}>${code.detailCodeName}</option>
                                    </c:forEach>
                                </select>
                            </td>
                            <th class="text-center" style="background-color: #e9ecef;">판매상태</th>
                            <td>
                                <select name="saleStatus" class="form-select form-select-sm" style="max-width: 200px;">
                                    <option value="">전체</option>
                                    <c:forEach var="code" items="${saleStatuses}">
                                        <option value="${code.detailCode}" ${param.saleStatus eq code.detailCode ? 'selected' : ''}>${code.detailCodeName}</option>
                                    </c:forEach>
                                </select>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <select name="divisionType" class="form-select form-select-sm" style="max-width: 200px;">
                                    <option value="">전체</option>
                                    <c:forEach var="code" items="${divisionTypes}">
                                        <option value="${code.detailCode}" ${param.divisionType eq code.detailCode ? 'selected' : ''}>${code.detailCodeName}</option>
                                    </c:forEach>
                                </select>
                            </td>
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="processType" class="form-select form-select-sm" style="max-width: 200px;">
                                    <option value="">전체</option>
                                    <c:forEach var="code" items="${processTypes}">
                                        <option value="${code.detailCode}" ${param.processType eq code.detailCode ? 'selected' : ''}>${code.detailCodeName}</option>
                                    </c:forEach>
                                </select>
                            </td>
            ...
            <tbody>
                <c:forEach var="item" items="${bundleList}" varStatus="status">
                    <tr>
                        ...
                        <td class="text-start">
                            <c:url var="bundleViewUrl" value="/admin/product/bundleView.do">
    ...
    <!-- 페이지 유지용 form -->
    <form name="pageForm" id="pageForm" method="get" action="<c:url value='/admin/product/bundleProductList.do'/>">
        <input type="hidden" name="menu" value="bundle" />
        <input type="hidden" name="pageIndex" value="${not empty paginationInfo ? paginationInfo.currentPageNo : 1}" />
