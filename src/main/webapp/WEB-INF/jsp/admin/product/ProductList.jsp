...
<div class="container-fluid p-4">
    ...
    <div class="card mb-4 border-0 shadow-sm">
        <div class="card-body p-0">
            <form name="searchForm" id="searchForm" action="<c:url value='/admin/product/productList.do'/>" method="get">
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="displayYn" class="form-select form-select-sm" style="max-width: 200px;">
                                    ...
                                    <option value="Y" <c:if test="${searchVO.displayYn eq 'Y'}">selected</c:if>>노출</option>
                                    <option value="N" <c:if test="${searchVO.displayYn eq 'N'}">selected</c:if>>비노출</option>
                                </select>
                            </td>
                            <th class="text-center" style="background-color: #e9ecef;">등록일</th>
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    <input type="date" name="searchStartDate" class="form-control form-control-sm"
                                        value="${searchVO.searchStartDate}" style="max-width: 150px;" />
                                    <span>~</span>
                                    <input type="date" name="searchEndDate" class="form-control form-control-sm"
                            ...
                            <td colspan="3">
                                <div class="d-flex gap-2">
                                    <input type="text" name="searchKeyword" class="form-control form-control-sm" 
                                        placeholder="상품명을 입력하세요" value="${searchVO.searchKeyword}" style="max-width: 400px;" />
                                </div>
                            </td>
                        </tr>
                    </tbody>
    ...
    <div class="d-flex justify-content-between align-items-center mb-2">
        ...
        <div class="d-flex gap-2">
            <button type="button" class="btn btn-success btn-sm" onclick="location.href='<c:url value='/admin/product/productRegist.do'/>'">
            ...
            <tbody>
                <c:forEach var="item" items="${productList}" varStatus="status">
                    <tr>
                        ...
                        <td>
                            <c:choose>
                                <c:when test="${not empty item.saleStartDate && item.saleStartDate.time > today.time}">
                                    <span class="badge bg-info">판매예정</span>
                                </c:when>
                                <c:when test="${not empty item.saleEndDate && item.saleEndDate.time < today.time}">
